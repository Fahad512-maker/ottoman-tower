@extends('layouts.buildapp' , ['class' => 'off-canvas-sidebar', 'category_name' => __('equity'), 'page_name' => __('create_equity')])
...
<div class="container">
                 ...
                 <div class="widget-content widget-content-area br-6 mt-5">
                    <form action="{{ url('/equity/store') }}"  method="POST" enctype="multipart/form-data">
                        @csrf
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Partner</label>
                                <select style="text-transform: capitalize" name="partner_name" id="" class="form-control" value="{{old('partner_name')}}" required>
                                  <option selected hidden disabled>Select Partner</option>
                                   @foreach($Partners as $value)
                                  <option style="text-transform: capitalize" value="{{$value->id}}">{{$value->name}}</option>
                                  @endforeach
                                </select>
                                @error('partner_name')
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Amount</label>
                                <input type="text" name="partner_amount"  class="form-control" placeholder="Enter Amount" value="{{old('partner_amount')}}" required>
                                @error('amount')
                                <span style="color: red">{{ $message }}</span>
                                @enderror
                            </div>
