@extends('layouts.buildapp' , ['class' => 'off-canvas-sidebar', 'category_name' => __('accounts'), 'page_name' => __('create_chartaccount')])
...
<div class="container">
                 ...
                 <div class="widget-content widget-content-area br-6 mt-5">
                    <form action="{{ url('/chartaccount/'.$chart->id.'/update') }}"  method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                    <div class="row mt-2">
                        <div class="col-md-6 mt-1">
                            <div class="form-group">
                                ...
                                <select name="account_type_id" id="" class="form-control">    
                                    ...
                                    <option value="{{$value->id}}" {{$chart->account_type_id == $value->id ? 'selected' : ''}} >{{$value->account_type}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6 mt-1">
                          ...
                          <input type="text"  name="account_name" placeholder="Enter Account" value="{{$chart->account_name}}" class="form-control">
                          @error('account_name')
                          <span style="color: red">{{ $message }}</span>
                          @enderror
                      </div>
