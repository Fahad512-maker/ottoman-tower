@extends('layouts.buildapp' , ['class' => 'off-canvas-sidebar', 'category_name' => __('smaterial'), 'page_name' => __('manage_supplier_payment')])
                            ...
                            <div class="statbox widget box box-shadow">
                                <div class="widget-header">                                
                                        ...
                                        <div class="col-xl-9 col-md-9 col-sm-9 col-9">
                                            <h4>Manage Supplier Payments</h4>
                                        </div>
                                        <div class="col-xl-3 col-md-3 col-sm-3 col-3">
                                           <a href="{{ url('/supplierpay/create') }}" class="btn btn-primary">Add Supplier Payments</a>
                                        </div>
                                        </div>                                                        
                                </div>
                 ...
                 <div class="widget-content widget-content-area br-6 mt-5">
                    <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                                <thead>
                                 ...
                                 <tbody>
                                    @if($supplierpay->isNotEmpty())
                                    @foreach($supplierpay as $key => $value)
                                    <tr>
                                        <td>{{ $key+1 }}</td>
                                        <td>{{ $value->supplierof->name}}</td>
                                        <td>{{ $value->projectof->projectname }}</td>
                                        <td>{{ $value->supplier_expanse }}</td>
                                        <td>{{ $value->supplier_paid}}</td> 
                                        <td>{{ $value->supplier_remaining}}</td> 
                                        
                                         {{--<td><img src="{{ asset($value->image) }}" class="img-fluid" width="100" height="100"></td> --}}
                                         <td><a href="{{ url('/supplierpay/'.$value->id.'/edit') }}"><i data-feather="edit"></i></a>
                                           <a href="javascript:void(0)" class="delete_option" data-id="{{ $value->id }}"><i data-feather="trash"></i></a>
                                           <a href="{{ url('/supplierpay/'.$value->id.'/print') }}" target="_blank" class="badge badge-primary" >Print</a>
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <a class="btn text-primary" data-dismiss="modal"><i class="flaticon-cancel-12"></i>Cancel</a>
                <a href="{{ url('/supplierpay/'.$value->id.'/delete') }}" class="btn btn-primary">Delete</a>
        </div>
      </div>
